...
@api_view(['GET', 'POST'])
def get_curious_fact(request):
    get_token_url = "https://tecweb-avaliacao-final.herokuapp.com/renatolf1/token"

    resposta = requests.request("GET", get_token_url)
    token = resposta.json()["token"]

    if request.method == 'POST':

        post_number = request.data["number"]

        post_url =  "https://tecweb-avaliacao-final.herokuapp.com/"

        headers = {"Authorization": f"Token {token}"}

        body = {"username": "renatolf1", "number": post_number}

        resposta_post = requests.request("POST", post_url, headers=headers, data=body)
        message_json = resposta_post.json()
        print(message_json)
